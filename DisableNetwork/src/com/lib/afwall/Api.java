...
import com.example.disablenetwork.MainActivity;
import com.example.disablenetwork.MainActivity.GetAppList;
import com.example.disablenetwork.R;
import com.lib.afwall.RootShell.RootCommand;
import com.lib.superuser.Shell.SU;
import com.lib.utils.JsonHelper;
...
public final class Api {
	...
	// iptables can exit with status 4 if two processes tried to update the same
	// table
	private static final int IPTABLES_TRY_AGAIN = 4;

	private static String AFWALL_CHAIN_NAME = "afwall";

	private static final String dynChains[] = { "-3g-postcustom", "-3g-fork",
			"-wifi-postcustom", "-wifi-fork" };
	...
	static void setIpTablePath(Context ctx, boolean setv6) {
		...
		if (pref.equals("system")) {
			...
			// auto setting:
			// IPv4 iptables on ICS+ devices is mostly sane, so we'll use it by
			// default
			// IPv6 ip6tables can return the wrong exit status (bug #215) so
			// default to our fixed version
	...
	/**
	 * Look up uid for each user by name, and if he exists, append an iptables
	 * rule.
	 * 
	 * @param listCommands
	 *            current list of iptables commands to execute
	 * @param users
	 *            list of users to whom the rule applies
	 * @param prefix
	 *            "iptables" command and the portion of the rule preceding
	 *            "-m owner --uid-owner X"
	 * @param suffix
	 *            the remainder of the iptables rule, following
	 *            "-m owner --uid-owner X"
	 */
	private static void addRuleForUsers(List<String> listCommands,
			String users[], String prefix, String suffix) {
		for (String user : users) {
			int uid = android.os.Process.getUidForName(user);
			if (uid != -1)
				listCommands.add(prefix + " -m owner --uid-owner " + uid + " "
						+ suffix);
		}
	}

	private static void addRulesForUidlist(List<String> cmds,
			List<Integer> uids, String chain, boolean whitelist) {
		String action = whitelist ? " -j RETURN" : " -j " + AFWALL_CHAIN_NAME
				+ "-reject";
		...
		if (uids.indexOf(SPECIAL_UID_ANY) >= 0) {
			...
			for (Integer uid : uids) {
				if (uid != null && uid >= 0) {
					cmds.add("-A " + chain + " -m owner --uid-owner " + uid
							+ action);
				}
			}
			...
			if (uids.indexOf(SPECIAL_UID_DNSPROXY) >= 0) {
				addRuleForUsers(cmds, new String[] { "root" }, "-A " + chain
						+ " -p udp --dport 53", action);
			}

			// NTP service runs as "system" user
			if (uids.indexOf(SPECIAL_UID_NTP) >= 0) {
				addRuleForUsers(cmds, new String[] { "system" }, "-A " + chain
						+ " -p udp --dport 123", action);
			}
			...
			if (whitelist) {
				if (kernel_checked) {
					...
					cmds.add("-A " + chain
							...
							+ AFWALL_CHAIN_NAME + "-reject");
				} else {
					// kernel is blocked so reject everything
					cmds.add("-A " + chain + " -j " + AFWALL_CHAIN_NAME
							+ "-reject");
				}
			} else {
				if (kernel_checked) {
					...
					cmds.add("-A " + chain
							+ " -m owner --uid-owner 0:999999999 -j RETURN");
					cmds.add("-A " + chain + " -j " + AFWALL_CHAIN_NAME
							+ "-reject");
				}
			}
		...
		if (G.enableLog()) {
			if (G.logTarget().equals("LOG")) {
				cmds.add("-A "
						+ AFWALL_CHAIN_NAME
						+ "-reject"
						+ " -m limit --limit 1000/min -j LOG --log-prefix \"{AFL}\" --log-level 4 --log-uid");
			} else if (G.logTarget().equals("NFLOG")) {
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-reject"
						+ " -j NFLOG --nflog-prefix \"{AFL}\" --nflog-group 40");
			}
		}
		cmds.add("-A " + AFWALL_CHAIN_NAME + "-reject" + " -j REJECT");
	}

	private static void addCustomRules(Context ctx, String prefName,
			List<String> cmds) {
	...
	/**
	 * Reconfigure the firewall rules based on interface changes seen at
	 * runtime: tethering enabled/disabled, IP address changes, etc. This should
	 * only affect a small number of rules; we want to avoid calling
	 * applyIptablesRulesImpl() too often since applying 100+ rules is
	 * expensive.
	 * 
	 * @param ctx
	 *            application context
	...
	private static void addInterfaceRouting(Context ctx, List<String> cmds) {
		...
		final boolean whitelist = G.pPrefs.getString(PREF_MODE, MODE_WHITELIST)
		...
		if (whitelist) {
			...
			addRuleForUsers(cmds, new String[] { "dhcp", "wifi" }, "-A "
					+ AFWALL_CHAIN_NAME + "-wifi-postcustom", "-j RETURN");
		}

		if (cfg.isTethered) {
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-postcustom -j "
					+ AFWALL_CHAIN_NAME + "-wifi-tether");
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g-postcustom -j "
					+ AFWALL_CHAIN_NAME + "-3g-tether");
		} else {
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-postcustom -j "
					+ AFWALL_CHAIN_NAME + "-wifi-fork");
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g-postcustom -j "
					+ AFWALL_CHAIN_NAME + "-3g-fork");
		}

		if (G.enableLAN() && !cfg.isTethered) {
			if (setv6 && !cfg.lanMaskV6.equals("")) {
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork -d "
						+ cfg.lanMaskV6 + " -j " + AFWALL_CHAIN_NAME
						+ "-wifi-lan");
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork '!' -d "
						+ cfg.lanMaskV6 + " -j " + AFWALL_CHAIN_NAME
						+ "-wifi-wan");
				...
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork -d "
						+ cfg.lanMaskV4 + " -j " + AFWALL_CHAIN_NAME
						+ "-wifi-lan");
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork '!' -d "
						+ cfg.lanMaskV4 + " -j " + AFWALL_CHAIN_NAME
						+ "-wifi-wan");
			} else {
				// No IP address -> no traffic. This prevents a data leak
				// between the time
				// the interface gets an IP address, and the time we process the
				// intent
				// (which could be 5+ seconds). This is likely to catch a little
				// bit of
				// legitimate traffic from time to time, so we won't log the
				// failures.
				cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork -j REJECT");
			}
		} else {
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-fork -j "
					+ AFWALL_CHAIN_NAME + "-wifi-wan");
		}

		if (G.enableRoam() && cfg.isRoaming) {
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g-fork -j "
					+ AFWALL_CHAIN_NAME + "-3g-roam");
		} else {
			cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g-fork -j "
					+ AFWALL_CHAIN_NAME + "-3g-home");
		}
	}
	...
	private static boolean applyIptablesRulesImpl(final Context ctx,
			...
			List<Integer> uidsRoam, List<Integer> uidsVPN,
			List<Integer> uidsLAN, final boolean showErrors, List<String> out) {
		if (ctx == null) {
			return false;
		}
		...
		if (G.isMultiUser()) {
			...
			AFWALL_CHAIN_NAME = AFWALL_CHAIN_NAME
					+ (G.getMultiUserId() != 0 ? G.getMultiUserId() : "");
		}
		...
		for (String s : dynChains) {
			cmds.add("#NOCHK# -N " + AFWALL_CHAIN_NAME + s);
			// addInterfaceRouting() will flush these chains, but not create
			// them
		}
		...
		cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g -j " + AFWALL_CHAIN_NAME
				+ "-3g-postcustom");
		cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi -j " + AFWALL_CHAIN_NAME
				+ "-wifi-postcustom");
		addRejectRules(cmds, ctx);

		if (G.enableInbound()) {
			// we don't have any rules in the INPUT chain prohibiting inbound
			// traffic, but
			// local processes can't reply to half-open connections without this
			// rule
		...
		// send wifi, 3G, VPN packets to the appropriate dynamic chain based on
		// interface
		if (G.enableVPN()) {
			// if !enableVPN then we ignore those interfaces (pass all traffic)
			for (final String itf : ITFS_VPN) {
				cmds.add("-A " + AFWALL_CHAIN_NAME + " -o " + itf + " -j "
						+ AFWALL_CHAIN_NAME + "-vpn");
			}
			// KitKat policy based routing - see:
			// http://forum.xda-developers.com/showthread.php?p=48703545
			// This covers mark range 0x3c - 0x47. The official range is
			// believed to be
			// 0x3c - 0x45 but this is close enough.
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
				cmds.add("-A " + AFWALL_CHAIN_NAME
						+ " -m mark --mark 0x3c/0xfffc -g " + AFWALL_CHAIN_NAME
						+ "-vpn");
				cmds.add("-A " + AFWALL_CHAIN_NAME
						+ " -m mark --mark 0x40/0xfff8 -g " + AFWALL_CHAIN_NAME
						+ "-vpn");
			}
		}
		for (final String itf : ITFS_WIFI) {
			cmds.add("-A " + AFWALL_CHAIN_NAME + " -o " + itf + " -j "
					+ AFWALL_CHAIN_NAME + "-wifi");
		}

		for (final String itf : ITFS_3G) {
			cmds.add("-A " + AFWALL_CHAIN_NAME + " -o " + itf + " -j "
					+ AFWALL_CHAIN_NAME + "-3g");
		}
		...
		// note that this can only blacklist DNS/DHCP services, not all tethered
		// traffic
		if (((!whitelist && (any_wifi || any_3g))
				|| (uids3g.indexOf(SPECIAL_UID_TETHER) >= 0) || (uidsWifi
				.indexOf(SPECIAL_UID_TETHER) >= 0))) {

			String users[] = { "root", "nobody" };
			String action = " -j "
					+ (whitelist ? "RETURN" : AFWALL_CHAIN_NAME + "-reject");

			// DHCP replies to client
			addRuleForUsers(cmds, users, "-A " + AFWALL_CHAIN_NAME
					+ "-wifi-tether", "-p udp --sport=67 --dport=68" + action);

			// DNS replies to client
			addRuleForUsers(cmds, users, "-A " + AFWALL_CHAIN_NAME
					+ "-wifi-tether", "-p udp --sport=53" + action);
			addRuleForUsers(cmds, users, "-A " + AFWALL_CHAIN_NAME
					+ "-wifi-tether", "-p tcp --sport=53" + action);

			// DNS requests to upstream servers
			addRuleForUsers(cmds, users, "-A " + AFWALL_CHAIN_NAME
					+ "-3g-tether", "-p udp --dport=53" + action);
			addRuleForUsers(cmds, users, "-A " + AFWALL_CHAIN_NAME
					+ "-3g-tether", "-p tcp --dport=53" + action);
		}

		// if tethered, try to match the above rules (if enabled). no match ->
		// fall through to the
		// normal 3G/wifi rules
		cmds.add("-A " + AFWALL_CHAIN_NAME + "-wifi-tether -j "
				+ AFWALL_CHAIN_NAME + "-wifi-fork");
		cmds.add("-A " + AFWALL_CHAIN_NAME + "-3g-tether -j "
				+ AFWALL_CHAIN_NAME + "-3g-fork");

		// NOTE: we still need to open a hole to let WAN-only UIDs talk to a DNS
		// server
		// on the LAN
		if (whitelist) {
			cmds.add("-A " + AFWALL_CHAIN_NAME
					+ "-wifi-lan -p udp --dport 53 -j RETURN");
		}

		// now add the per-uid rules for 3G home, 3G roam, wifi WAN, wifi LAN,
		// VPN
		// in whitelist mode the last rule in the list routes everything else to
		// afwall-reject
		addRulesForUidlist(cmds, uids3g, AFWALL_CHAIN_NAME + "-3g-home",
				whitelist);
		addRulesForUidlist(cmds, uidsRoam, AFWALL_CHAIN_NAME + "-3g-roam",
				whitelist);
		addRulesForUidlist(cmds, uidsWifi, AFWALL_CHAIN_NAME + "-wifi-wan",
				whitelist);
		addRulesForUidlist(cmds, uidsLAN, AFWALL_CHAIN_NAME + "-wifi-lan",
				whitelist);
	 ...
	 * 
	 * @param in
	 *            Commands in the format: "-A foo ...", "#NOCHK# -A foo ...", or
	 *            "#LITERAL# <UNIX command>"
	 * @param out
	 *            A list of UNIX commands to execute
	 */
	...
	private static List<Integer> getUidListFromPref(Context ctx,
			final String pks) {
		initSpecial();
	 ...
	 *            indicates if errors should be alerted
	 * @param callback
	 *            If non-null, use a callback instead of blocking the current
	 *            thread
	 */
	public static boolean applySavedIptablesRules(Context ctx,
			boolean showErrors, RootCommand callback) {
		if (ctx == null) {
			return false;
		}
		initSpecial();
		...
		final String savedPkg_roam_uid = G.pPrefs.getString(
		...
		returnValue = applyIptablesRulesImpl(ctx,
				...
				getListFromPref(savedPkg_lan_uid), showErrors, cmds);
		...
		try {
			...
			if (callback != null) {
				...
				if (runScriptAsRoot(ctx, out, new StringBuilder()) == -1) {
					if (showErrors)
						alert(ctx, ctx.getString(R.string.error_purge));
					return false;
				}
			}
	 ...
	 *            application context
	 * @param callback
	 *            callback to receive rule list
	 * @param useIPV6
	 *            true to list IPv6 rules, false to list IPv4 rules
	 */
	public static void fetchIptablesRules(Context ctx, boolean useIPV6,
			RootCommand callback) {
		List<String> cmds = new ArrayList<String>();
		List<String> out = new ArrayList<String>();
		cmds.add("-n -v -L");
	 ...
	 */
	public static void apply46(Context ctx, List<String> cmds,
			RootCommand callback) {
		List<String> out = new ArrayList<String>();
	...
	public static List<PackageInfoData> getApps(Context ctx, GetAppList appList) {
		...
		final String savedPkg_wifi_uid = prefs
				.getString(PREF_WIFI_PKG_UIDS, "");
		...
		// always use the defaul preferences to store cache value - reduces the
		// application usage size
		final SharedPreferences cachePrefs = ctx.getSharedPreferences(
				"AFWallPrefs", Context.MODE_PRIVATE);
		...
		try {
			...
			specialData.add(new PackageInfoData(SPECIAL_UID_ANY, ctx
					.getString(R.string.all_item), "dev.afwall.special.any"));
			specialData.add(new PackageInfoData(SPECIAL_UID_KERNEL, ctx
					.getString(R.string.kernel_item),
					"dev.afwall.special.kernel"));
			specialData.add(new PackageInfoData(SPECIAL_UID_TETHER, ctx
					.getString(R.string.tethering_item),
					"dev.afwall.special.tether"));
			specialData.add(new PackageInfoData(SPECIAL_UID_DNSPROXY, ctx
					.getString(R.string.dnsproxy_item),
					"dev.afwall.special.dnsproxy"));
			specialData.add(new PackageInfoData(SPECIAL_UID_NTP, ctx
					.getString(R.string.ntp_item), "dev.afwall.special.ntp"));
			specialData.add(new PackageInfoData("root", ctx
					.getString(R.string.root_item), "dev.afwall.special.root"));
			specialData.add(new PackageInfoData("media", "Media server",
					"dev.afwall.special.media"));
			specialData.add(new PackageInfoData("vpn", "VPN networking",
					"dev.afwall.special.vpn"));
			specialData.add(new PackageInfoData("shell", "Linux shell",
					"dev.afwall.special.shell"));
			specialData.add(new PackageInfoData("gps", "GPS",
					"dev.afwall.special.gps"));
			specialData.add(new PackageInfoData("adb",
					"ADB (Android Debug Bridge)", "dev.afwall.special.adb"));
	...
	// private static List<String> getListFromPref(Context ctx, String
	// savedPkg_uid) {
	// final PackageManager pm = ctx.getPackageManager();
	//
	...
	private static class RunCommand extends
			AsyncTask<Object, List<String>, Integer> {
	 ...
	 */
	public static int runScriptAsRoot(Context ctx, List<String> script,
		...
		if ((Looper.myLooper() != null)
			...
			Log.e(TAG,
					"runScriptAsRoot should not be called from the main thread\nCall Trace:\n");
	...
	private static boolean migrateSettings(Context ctx, int lastVer,
			int currentVer) {
		if (lastVer <= 138) {
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			currentVer = ctx.getPackageManager().getPackageInfo(
		...
		Log.d(TAG, "binary installation for " + abi
				+ (ret ? " succeeded" : " failed"));

		if (showErrors) {
			if (ret) {
				displayToasts(ctx, R.string.toast_bin_installed,
						Toast.LENGTH_LONG);
			} else {
		...
		if (currentVer > 0) {
			if (migrateSettings(ctx, lastVer, currentVer) == false
					&& showErrors) {
		...
		if (ret == true && currentVer > 0) {
			// this indicates that migration from the old version was
			// successful.
	...
	public static boolean isEnabled(Context ctx) {
		if (ctx == null)
			return false;
		boolean flag = ctx.getSharedPreferences(PREF_FIREWALL_STATUS,
				Context.MODE_PRIVATE).getBoolean(PREF_ENABLED, false);
	...
	public static void setEnabled(Context ctx, boolean enabled,
		...
		final SharedPreferences prefs = ctx.getSharedPreferences(
				PREF_FIREWALL_STATUS, Context.MODE_PRIVATE);
	...
	private static boolean removePackageRef(Context ctx, String pkg,
			int pkgRemoved, Editor editor, String store) {
	...
	/**
	 * Remove the cache.label key from preferences, so that next time the app
	 * appears on the top
	 * 
	 * @param pkgName
	 * @param ctx
	...
	/**
	 * Cleansup the uninstalled packages from the cache - will have slight
	 * performance
	 * 
	 * @param ctx
	 */
	public static void removeAllUnusedCacheLabel(Context ctx) {
		...
		for (Map.Entry<String, ?> entry : keys.entrySet()) {
			if (entry.getKey().startsWith(cacheLabel)) {
				...
				if (prefs.getString(cacheKey, "").length() > 0
						&& !isPackageExists(pm, pkgName)) {
					prefs.edit().remove(cacheKey).commit();
				}
			}
		}
	...
	public static boolean isPackageExists(PackageManager pm,
	...
	public static void applicationRemoved(Context ctx, int pkgRemoved) {
		...
		wChanged = removePackageRef(ctx, savedPks_wifi, pkgRemoved, editor,
				PREF_WIFI_PKG_UIDS);
		// look for the removed application in the "3g" list
		gChanged = removePackageRef(ctx, savedPks_3g, pkgRemoved, editor,
				PREF_3G_PKG_UIDS);
		// look for the removed application in roaming list
		rChanged = removePackageRef(ctx, savedPks_roam, pkgRemoved, editor,
				PREF_ROAMING_PKG_UIDS);
		// look for the removed application in vpn list
		vChanged = removePackageRef(ctx, savedPks_vpn, pkgRemoved, editor,
				PREF_VPN_PKG_UIDS);
		// look for the removed application in lan list
		vChanged = removePackageRef(ctx, savedPks_lan, pkgRemoved, editor,
				PREF_LAN_PKG_UIDS);
	...
	/*
	 * public static String runSUCommand(String cmd) throws IOException { final
	 * StringBuilder res = new StringBuilder(); Process p =
	 * Runtime.getRuntime().exec( new String[] { "su", "-c", cmd });
	 * BufferedReader stdout = new BufferedReader(new
	 * InputStreamReader(p.getInputStream())); String tmp; while ((tmp =
	 * stdout.readLine()) != null) { res.append(tmp); res.append(","); } // use
	 * inputLine.toString(); here it would have whole source stdout.close();
	 * return res.toString(); }
	 */
						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int id) {
								if (saveSharedPreferencesToFile(ctx)) {
									Api.alert(
											ctx,
											ctx.getString(R.string.export_rules_success)
													+ " "
													+ Environment
									...
									Api.alert(
											ctx,
											ctx.getString(R.string.export_rules_fail));
								}
							}
						})
	...
	private static void updateExportPackage(Map<String, JSONObject> exportMap,
			String packageName, int identifier) throws JSONException {
		JSONObject obj;
		if (packageName != null) {
	...
	public static boolean saveSharedPreferencesToFile(Context ctx) {
		...
		if (isExternalStorageWritable()) {
			...
			try {
				...
				for (int i = 0; i < apps.size(); i++) {
					if (apps.get(i).selected_wifi) {
						updateExportPackage(exportMap, apps.get(i).pkgName,
								WIFI_EXPORT);
					}
					if (apps.get(i).selected_3g) {
						updateExportPackage(exportMap, apps.get(i).pkgName,
	...
	public static boolean loadSharedPreferencesFromFile(Context ctx) {
		...
		try {
			...
			for (Map.Entry<String, Object> entry : json.entrySet()) {
				...
				JSONObject jsonObj = (JSONObject) JsonHelper.toJSON(entry
						.getValue());
				...
				while (keys.hasNext()) {
					...
					case DATA_EXPORT:
						...
						if (pkgName.startsWith("dev.afwall.special")) {
							...
							data_uids
									.append(pm.getApplicationInfo(pkgName, 0).uid);
						}
						break;
					case ROAM_EXPORT:
		...
		try {
			for (File f : new File("/sys/class/net").listFiles()) {
				...
				if (!showMatches) {
					...
					if (InterfaceTracker.matchName(InterfaceTracker.ITFS_WIFI,
							name) != null) {
						ret.add(name + ": wifi");
					} else if (InterfaceTracker.matchName(
							InterfaceTracker.ITFS_3G, name) != null) {
						ret.add(name + ": 3G");
					} else if (InterfaceTracker.matchName(
							InterfaceTracker.ITFS_VPN, name) != null) {
						ret.add(name + ": VPN");
					} else {
			...
			Log.e(TAG,
					"can't list network interfaces: " + e.getLocalizedMessage());
		}
		return ret;
	}
	...
	private static class LogProbeCallback extends RootCommand.Callback {
		...
		public void cbFunc(RootCommand state) {
			...
			if (hasLOG) {
				...
				displayToasts(ctx, R.string.log_target_failed,
						Toast.LENGTH_SHORT);
				G.logTarget("");
				G.enableLog(false);
				return;
	...
	public static void setLogging(final Context ctx, boolean isEnabled) {
		if (!isEnabled) {
			...
			G.enableLog(false);
			G.logTarget("");
			updateLogRules(ctx, new RootCommand().setReopenShell(true)
		...
		cb.ctx = ctx;
		// probe for LOG/NFLOG targets (unfortunately the file must be read by
		// root)
		new RootCommand().setReopenShell(true)
				.setFailureToast(R.string.log_toggle_failed).setCallback(cb)
		...
		if (!isRoot) {
			try {
				...
				if (returnCode == 0) {
					...
					Api.showAlertDialogActivity(ctx,
							ctx.getString(R.string.error_common),
							ctx.getString(R.string.error_su));
				}
			} catch (Exception e) {
			}
		}
	...
	public static void showAlertDialogActivity(Context ctx, String title,
			String message) {
	...
	private static void initSpecial() {
		if (specialApps == null || specialApps.size() == 0) {
			...
			specialApps
					.put("dev.afwall.special.dnsproxy", SPECIAL_UID_DNSPROXY);
			specialApps.put("dev.afwall.special.ntp", SPECIAL_UID_NTP);
			specialApps.put("dev.afwall.special.root",
					android.os.Process.getUidForName("root"));
			specialApps.put("dev.afwall.special.media",
					android.os.Process.getUidForName("media"));
			specialApps.put("dev.afwall.special.vpn",
					android.os.Process.getUidForName("vpn"));
			specialApps.put("dev.afwall.special.shell",
					android.os.Process.getUidForName("shell"));
			specialApps.put("dev.afwall.special.gps",
					android.os.Process.getUidForName("gps"));
			specialApps.put("dev.afwall.special.adb",
					android.os.Process.getUidForName("adb"));
		}
	}
	...
	public static void sendNotification(Context context, String title,
		...
		builder.setSmallIcon(icon).setWhen(System.currentTimeMillis())
	...
	public static boolean supportsMultipleUsers(Context context) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
			final UserManager um = (UserManager) context
					.getSystemService(Context.USER_SERVICE);
			try {
				Method supportsMultipleUsers = UserManager.class
						.getMethod("supportsMultipleUsers");
